...
#'
#' @details
#' Stage 3 of Multiple Imputation. The input is an array with p rows referring to the number of parameters to be combined. An estimate
...
#'
#' @note  Modified the \code{\link[mice]{pool.scalar}} (version R 3.4) in the \pkg{mice} package to handle multiple parameters at once in an array and combine them. Similar to \code{\link[norm]{mi.inference}} in the \pkg{norm} package, but the small-sample adjustment is missing.
#'
#' @references
...
pool.mi <- function(to.pool, n = 999999, method = c("smallsample", "rubin"), alpha = 0.05, verbose = FALSE) {
  method <- match.arg(method)
    ...
    if (is.null(n)) warning("Large sample assumed.")
